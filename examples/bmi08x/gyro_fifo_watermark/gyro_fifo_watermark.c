...
/* Watermark level */
#define BMI08_FIFO_WATERMARK_LEVEL             UINT8_C(50)
...
static int8_t enable_bmi08_interrupt()
{
    ...
    rslt = bmi08g_enable_watermark(BMI08_ENABLE, &bmi08dev);
    bmi08_error_codes_print_result("bmi08g_enable_watermark", rslt);
...
static int8_t disable_bmi08_interrupt()
{
    ...
    rslt = bmi08g_enable_watermark(BMI08_DISABLE, &bmi08dev);
    bmi08_error_codes_print_result("bmi08g_enable_watermark", rslt);
    ...
    if (rslt == BMI08_OK)
    {
        ...
        if (rslt == BMI08_OK)
        {
            ...
            if (rslt == BMI08_OK)
            {
                ...
                gyr_conf.wm_level = BMI08_FIFO_WATERMARK_LEVEL;
                gyr_conf.tag = BMI08_GYRO_FIFO_TAG_ENABLED;
                ...
                while (try <= 3)
                {
                    ...
                    if (status & BMI08_GYRO_FIFO_WM_INT)
                    {
                        printf("\nIteration : %d\n", try);
                        printf("Watermark level : %d (Indicates frame number) \n\n", gyr_conf.wm_level);
                        ...
                        printf("FIFO buffer size : %d\n", BMI08_FIFO_RAW_DATA_USER_LENGTH);
                        ...
                        {
                            if (((bmi08_gyro[idx].x) != 0) && ((bmi08_gyro[idx].y) != 0) && ((bmi08_gyro[idx].z) != 0))
                            {
                                printf("%d, %d, %d, %d\n", idx, bmi08_gyro[idx].x, bmi08_gyro[idx].y,
                                       bmi08_gyro[idx].z);
