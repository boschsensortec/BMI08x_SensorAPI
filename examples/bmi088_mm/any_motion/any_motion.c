...
static void configure_bmi08_any_motion_interrupt(struct bmi08_dev *bmi08dev)
{
    int8_t rslt;
    struct bmi088_mm_anymotion_cfg any_motion_cfg = { 0 };
    ...
    if (rslt == BMI08_OK)
    {
        ...
        no_motion_int_cfg.int_type = BMI088_MM_ANYMOTION_INT;
        ...
        rslt = bmi088_mma_set_int_config(&no_motion_int_cfg, BMI088_MM_ANYMOTION_INT, bmi08dev);
...
int main(void)
{
    ...
    configure_bmi08_any_motion_interrupt(&bmi08);
    ...
    for (;;)
    {
        ...
        if (status & BMI088_MM_ACCEL_ANY_MOT_INT)
        {
