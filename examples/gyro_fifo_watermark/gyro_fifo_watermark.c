...
/* Watermark level */
#define BMI08X_FIFO_WATERMARK_LEVEL             UINT8_C(50)
...
static int8_t enable_bmi08x_interrupt()
{
    ...
    rslt = bmi08g_enable_watermark(BMI08X_ENABLE, &bmi08xdev);
    bmi08x_error_codes_print_result("bmi08g_enable_watermark", rslt);
...
static int8_t disable_bmi08x_interrupt()
{
    ...
    rslt = bmi08g_enable_watermark(BMI08X_DISABLE, &bmi08xdev);
    bmi08x_error_codes_print_result("bmi08g_enable_watermark", rslt);
    ...
    if (rslt == BMI08X_OK)
    {
        ...
        if (rslt == BMI08X_OK)
        {
            ...
            if (rslt == BMI08X_OK)
            {
                ...
                gyr_conf.wm_level = BMI08X_FIFO_WATERMARK_LEVEL;
                gyr_conf.tag = BMI08X_GYRO_FIFO_TAG_ENABLED;
                ...
                while (try <= 3)
                {
                    ...
                    if (status & BMI08X_GYRO_FIFO_WM_INT)
                    {
                        printf("\nIteration : %d\n", try);
                        ...
                        printf("FIFO buffer size : %d\n", BMI08X_FIFO_RAW_DATA_USER_LENGTH);
                        ...
                        {
                            if (((bmi08x_gyro[idx].x) != 0) && ((bmi08x_gyro[idx].y) != 0) &&
                                ((bmi08x_gyro[idx].z) != 0))
                            {
