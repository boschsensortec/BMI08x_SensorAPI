 ...
 * @brief   Example file how to read bmi08x accel FIFO watermark data
 *
 */
...
#define BMI08X_ACC_FIFO_WATERMARK_LEVEL                UINT8_C(500)
 ...
 */
#define BMI08X_ACC_FIFO_WM_EXTRACTED_DATA_FRAME_COUNT  UINT8_C(75)
...
static int8_t enable_bmi08x_interrupt()
{
    ...
    accel_int_config.int_channel = BMI08X_INT_CHANNEL_2;
...
int main(void)
{
    ...
    uint16_t accel_length = BMI08X_ACC_FIFO_WM_EXTRACTED_DATA_FRAME_COUNT;
    ...
    if (rslt == BMI08X_OK)
    {
        ...
        if (rslt == BMI08X_OK)
        {
            ...
            while (try <= 3)
            {
                ...
                if (status & BMI08X_ACCEL_FIFO_WM_INT)
                {
                    printf("\nIteration : %d\n", try);

                    accel_length = BMI08X_ACC_FIFO_WM_EXTRACTED_DATA_FRAME_COUNT;

                    rslt = bmi08a_get_fifo_length(&fifo_length, &bmi08xdev);
                    bmi08x_error_codes_print_result("bmi08a_get_fifo_length", rslt);

                    rslt = bmi08a_get_fifo_wm(&wml, &bmi08xdev);
                    bmi08x_error_codes_print_result("bmi08a_get_fifo_length", rslt);
